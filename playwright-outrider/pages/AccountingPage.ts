...
export class AccountingPage {
  ...
  async clickPreviousMonth(): Promise<void> {
    try {
      await this.previousMonthButton.waitFor({ state: 'visible', timeout: TIMEOUTS.ELEMENT_WAIT });
  ...
  async clickNextMonth(): Promise<void> {
    try {
      await this.nextMonthButton.waitFor({ state: 'visible', timeout: TIMEOUTS.ELEMENT_WAIT });
   ...
   */
  async clickCalendarPicker(): Promise<void> {
    try {
      await this.calendarPicker.waitFor({ state: 'visible', timeout: TIMEOUTS.ELEMENT_WAIT });
      await this.calendarPicker.click();
    } catch (error) {
      throw error;
    }
  }
  ...
  /**
   * Click select month option
   */
  async clickSelectMonth(): Promise<void> {
    try {
      await this.selectMonth.waitFor({ state: 'visible', timeout: TIMEOUTS.ELEMENT_WAIT });
  ...
  /**
   * Click success payroll indicator
   */
  async clickSuccessPayroll(): Promise<void> {
    try {
      await this.successPayroll.waitFor({ state: 'visible', timeout: TIMEOUTS.ELEMENT_WAIT });
      await this.successPayroll.click();
    } catch (error) {
      throw error;
    }
  }
  ...
  /**
   * Click failed payroll indicator
   */
  async clickFailedPayroll(): Promise<void> {
    try {
      await this.failedPayroll.waitFor({ state: 'visible', timeout: TIMEOUTS.ELEMENT_WAIT });
      await this.failedPayroll.click();
    } catch (error) {
      throw error;
    }
  }
  ...
  /**
   * Click in-progress payroll indicator
   */
  async clickInProgressPayroll(): Promise<void> {
    try {
      await this.inProgressPayroll.waitFor({ state: 'visible', timeout: TIMEOUTS.ELEMENT_WAIT });
      await this.inProgressPayroll.click();
    } catch (error) {
      throw error;
    }
  }
  ...
  /**
   * Click connections selector
   */
  async clickConnectionsSelector(): Promise<void> {
    try {
      await this.connectionsSelector.waitFor({ state: 'visible', timeout: TIMEOUTS.ELEMENT_WAIT });
      await this.connectionsSelector.click();
    } catch (error) {
      throw error;
    }
  }
   ...
   * Select QBO company
   */
  async selectQBOCompany(): Promise<void> {
    try {
      await this.qboCompanyOption.waitFor({ state: 'visible', timeout: TIMEOUTS.ELEMENT_WAIT });
      await this.qboCompanyOption.click();
    } catch (error) {
      throw error;
    }
  }
  ...
  /**
   * Click paygroup filter
   */
  async clickPaygroupFilter(): Promise<void> {
    try {
      await this.paygroupFilter.waitFor({ state: 'visible', timeout: TIMEOUTS.ELEMENT_WAIT });
      await this.paygroupFilter.click();
    } catch (error) {
      throw error;
    }
  }
  ...
  async clickClearFilter(): Promise<void> {
    try {
      await this.clearFilterButton.waitFor({ state: 'visible', timeout: TIMEOUTS.ELEMENT_WAIT });
  ...
  /**
   * Search in paygroup filter
   */
  async searchPaygroup(searchText: string): Promise<void> {
    try {
      await this.paygroupSearchInput.waitFor({ state: 'visible', timeout: TIMEOUTS.ELEMENT_WAIT });
      await this.paygroupSearchInput.fill(searchText);
    } catch (error) {
      throw error;
    }
  }
  ...
  async clearSearchInput(): Promise<void> {
    try {
      await this.paygroupSearchInput.waitFor({ state: 'visible', timeout: TIMEOUTS.ELEMENT_WAIT });
      await this.paygroupSearchInput.clear();
    } catch (error) {
      throw error;
    }
  }
  ...
  /**
   * Click select all filter option
   */
  async clickSelectAllFilter(): Promise<void> {
    try {
      await this.selectAllFilter.waitFor({ state: 'visible', timeout: TIMEOUTS.ELEMENT_WAIT });
      await this.selectAllFilter.click();
    } catch (error) {
      throw error;
    }
  }
   ...
   */
  async clickSelectOption1(): Promise<void> {
    try {
      await this.selectOption1.waitFor({ state: 'visible', timeout: TIMEOUTS.ELEMENT_WAIT });
      await this.selectOption1.click();
    } catch (error) {
      throw error;
    }
  }
   ...
   */
  async clickSelectOption2(): Promise<void> {
    try {
      await this.selectOption2.waitFor({ state: 'visible', timeout: TIMEOUTS.ELEMENT_WAIT });
  ...
  async clickNotifications(): Promise<void> {
    try {
      await this.notificationsButton.waitFor({ state: 'visible', timeout: TIMEOUTS.ELEMENT_WAIT });
  ...
  async closeModal(): Promise<void> {
    try {
      await this.modalCloseButton.waitFor({ state: 'visible', timeout: TIMEOUTS.ELEMENT_WAIT });
  ...
  async openNotificationsPanel(): Promise<void> {
    try {
      ...
      // Try to wait for the panel, but don't fail if it doesn't appear with expected selector
      try {
        await this.notificationsPanel.waitFor({ state: 'visible', timeout: 5000 });
      } catch (panelError) {
        // Panel may have opened with different structure
      }
    } catch (error) {
      throw error;
    }
  ...
  async closeNotificationsPanel(): Promise<void> {
    try {
      await this.notificationsClose.waitFor({ state: 'visible', timeout: 5000 });
      await this.notificationsClose.click();
    } catch (error) {
  ...
  async clickMarkAllRead(): Promise<void> {
    try {
      ...
      // Try to click the mark all read button
      try {
        await this.markAllReadButton.waitFor({ state: 'visible', timeout: 5000 });
        await this.markAllReadButton.click();
      } catch (buttonError) {
        // Try to find the button with a more generic selector
        const genericMarkAllRead = this.page.locator('//span[contains(text(),"Mark all as read")]');
        if (await genericMarkAllRead.isVisible()) {
          await genericMarkAllRead.click();
        } else {
          throw new Error('Mark all read button not found with any selector');
        }
      }
    } catch (error) {
      throw error;
  ...
  /**
   * Verify accounting tab is not visible (for non-admin users)
   */
  async verifyAccountingTabNotVisible(): Promise<void> {
    try {
      const isVisible = await this.accountingTab.isVisible();
      if (isVisible) {
        throw new Error('Accounting tab should not be visible for non-admin user');
      }
    } catch (error) {
      throw error;
    }
  ...
  /**
   * Click notifications All filter button
   */
  async clickNotificationsAllFilter(): Promise<void> {
    try {
      await this.notificationsAllButton.waitFor({ state: 'visible', timeout: TIMEOUTS.ELEMENT_WAIT });
      await this.notificationsAllButton.click();
    } catch (error) {
      throw new Error(`Failed to click notifications All filter: ${error instanceof Error ? error.message : String(error)}`);
    }
  }

  /**
   * Click notifications Error filter button
   */
  async clickNotificationsErrorFilter(): Promise<void> {
    try {
      await this.notificationsErrorButton.waitFor({ state: 'visible', timeout: TIMEOUTS.ELEMENT_WAIT });
      await this.notificationsErrorButton.click();
    } catch (error) {
      throw new Error(`Failed to click notifications Error filter: ${error instanceof Error ? error.message : String(error)}`);
    }
  }

  /**
   * Click notifications Warning filter button
   */
  async clickNotificationsWarningFilter(): Promise<void> {
    try {
      await this.notificationsWarningButton.waitFor({ state: 'visible', timeout: TIMEOUTS.ELEMENT_WAIT });
      await this.notificationsWarningButton.click();
    } catch (error) {
      throw new Error(`Failed to click notifications Warning filter: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
